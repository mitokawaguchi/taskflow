...
import { getSupabase, getOwnerId, ensureCanDelete, clientFromRow } from './helpers'
import type { Client } from '../types'

export async function fetchClients(): Promise<Client[]> {
  const db = getSupabase()
  const ownerId = await getOwnerId()
  let q = db.from('tf_clients').select('*')
  if (ownerId) q = q.eq('owner_id', ownerId)
  const { data, error } = await q.order('id')
  if (error) throw error
  const rows = (data ?? []) as Array<Record<string, unknown>>
  return rows.map((row) => clientFromRow((row as unknown) as Parameters<typeof clientFromRow>[0])).filter((c): c is Client => c != null)
}
...
export async function insertClient(client: ClientInsertInput): Promise<Client> {
  ...
  const { data, error } = await db.from('tf_clients').insert(row).select().single()
  if (error) throw error
  const result = clientFromRow((data as unknown) as Parameters<typeof clientFromRow>[0])
...
export async function updateClient(id: string, patch: ClientUpdatePatch): Promise<Client> {
  ...
  const { data, error } = await db.from('tf_clients').update(row).eq('id', id).select().single()
  if (error) throw error
  const result = clientFromRow((data as unknown) as Parameters<typeof clientFromRow>[0])
...
export async function deleteClient(id: string): Promise<void> {
  ...
  const { error } = await db.from('tf_clients').delete().eq('id', id)
  if (error) throw error
}
